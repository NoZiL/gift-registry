...
// clears them, so both go through here (and it is exported so a test can check
// the pair line up): whatever a sheet doesn't have a column
// for is simply not written, and undoing can't leave half a claim behind. The
// name and the timestamp rarely sit next to each other, so these are separate
// ranges rather than one contiguous block.
