...
import { normalizeImageUrl } from "./preview";
...
// G: Category   H: Price   I: Image
//
// G, H and I were added after the first sheets were already in use, so they
// sit at the end rather than next to Item: an existing sheet keeps working
// untouched (they all read as empty), and the reserve write below still lands
// on D:F. Filling them in is what turns the filters, the grouped sections and
// the item thumbnails on.
const RANGE_ALL = "A2:I";
  ...
  return rows
    .map((row, idx) => {
      ...
      return {
        ...
        // Vetted here rather than on the card, so a cell holding something
        // that isn't an image URL never reaches the browser at all.
...
export async function releaseItem(id, name) {
  ...
  if (!sameGuest(reservedBy, name)) {
    return { ok: false, reason: "not_yours", reservedBy: reservedBy || "" };
  }
