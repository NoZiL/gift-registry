...
// G: Price (optional — see lib/price.js; the imported list keeps its price in
// the Notes text, so column G is only there for lists that want a clean one)
const RANGE_ALL = "A2:G";
...
export async function getItems() {
  ...
  return rows
    .map((row, idx) => {
      const [item, link, notes, reserved, reservedBy, reservedAt, price] = row;
      ...
      return {
        ...
        price: resolvePrice({ price, notes }), // number, or null when unknown
      };
    })
    .filter(Boolean);
}
