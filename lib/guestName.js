...
// A name typed on a phone, a name pasted into a `?g=` link and a name saved
// months ago are all the same guest, so releasing a reservation shouldn't hinge
// on the capitals or the accent being reproduced exactly. Still an exact match
// on everything that actually distinguishes two people.
...
export function sameGuest(a, b) {
  ...
  // An empty name matches nobody, least of all an unclaimed row.
